...
- 组件录入词库：类别、品名和品牌自动记忆，可在词库设置中提前维护、停用，并在新建组件时模糊搜索复用。
- 配置单输出：保存后的自选配置可导出 A4 横向 PDF、带中文表头的 CSV，并从最终 PDF 打印。
...
1. 创建加密工作区并录入公司名称。
2. 建立产品、组件、客户和供应商；需要组合报价时创建自选配置产品。
...
- 数据库：SQLCipher Schema V8，客户档案、自选配置与组件词库通过增量迁移兼容旧数据，销售单证字段保存在向后兼容的 JSON 快照中。
