...
- 加密附件库：合同附件、采购确认文件和生产资料直接保存到 SQLCipher，并记录文件大小与 SHA-256。
...
8. 在“数据与安全”创建 `.tdbackup` 加密备份；需要恢复时先锁定工作区，再在解锁页选择备份文件。

“打印”会先生成与预览一致的最终 PDF，再使用系统默认 PDF 阅读器打开。

## 数据与版本

- 数据库：SQLCipher Schema V11，客户档案、自选配置、汇率快照、多语组件词库、业务单产品来源、附件和自动草稿通过增量迁移兼容旧数据，销售单证字段保存在向后兼容的 JSON 快照中。
