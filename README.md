...
- 企业单证标识：在“企业设置”维护公司名称、Logo 和电子签名，资料保存在 SQLCipher 加密工作区，统一应用到配置单、报价、发票、箱单和合同 PDF 的页眉、签名区与页脚。
...
1. 创建加密工作区，在“企业设置”录入公司名称并按需上传 Logo 和电子签名。
...
- 数据库：SQLCipher Schema V10，客户档案、自选配置、汇率快照、多语组件词库和业务单产品来源通过增量迁移兼容旧数据，销售单证字段保存在向后兼容的 JSON 快照中。
