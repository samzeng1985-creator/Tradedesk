  ...
  const targets: DocumentType[] = source.documentType === "commercial_quotation"
    ...
    : source.documentType === "proforma_invoice"
      ...
      : source.documentType === "commercial_invoice"
        ? ["packing_list", "shipping_marks", "shipper_instruction", "customs_declaration", "bill_of_lading", "insurance_policy", "certificate_of_origin", "inspection_certificate"]
        : source.documentType === "packing_list"
          ? ["shipping_marks", "shipper_instruction", "customs_declaration", "bill_of_lading", "insurance_policy", "certificate_of_origin", "inspection_certificate"]
          : source.documentType === "shipper_instruction"
            ? ["bill_of_lading", "insurance_policy", "certificate_of_origin", "inspection_certificate"]
            : source.documentType === "customs_declaration"
              ? ["certificate_of_origin", "inspection_certificate"]
              : ["insurance_policy", "certificate_of_origin", "inspection_certificate"];
...
function Preview({ document, payload, company, signingAsset }: { document: TradeDocument; payload: DocumentPayload; company?: CompanyRecord; signingAsset?: CompanySigningAsset }) {
  ...
  const packing = ["packing_list", "shipping_marks", "shipper_instruction", "customs_declaration", "bill_of_lading", "certificate_of_origin", "inspection_certificate"].includes(document.documentType);
...
function DocumentEditor({ initial, companyRegistry, onClose, onSave, onIssue, onExportPdf, onExportCsv, onPrint }: {
  ...
  const amountDocument = !["packing_list", "shipping_marks", "shipper_instruction", "bill_of_lading", "certificate_of_origin", "inspection_certificate"].includes(document.documentType);
  ...
  return <div className="document-editor-shell">
    ...
    <div className="document-editor-layout">
      <div className="document-form-panel">
        ...
        {(certificateOfOrigin || inspectionCertificate) && <><h3>认证资料</h3><div className="form-grid two-columns"><label>运输方式<input value={payload.transportMode} onChange={(event) => setPayloadField({ transportMode: event.target.value })} /></label><label>船名/航次<input value={payload.vesselVoyage} onChange={(event) => setPayloadField({ vesselVoyage: event.target.value })} /></label><label>证书号/申请号<input value={payload.certificateNumber} onChange={(event) => setPayloadField({ certificateNumber: event.target.value })} /></label><label>{certificateOfOrigin ? "签证机构" : "检验机构"}<input value={payload.certificationAuthority} onChange={(event) => setPayloadField({ certificationAuthority: event.target.value })} /></label>{certificateOfOrigin && <><label>证书类型<select value={payload.certificateType} onChange={(event) => setPayloadField({ certificateType: event.target.value })}><option>General Certificate of Origin</option><option>Form E</option><option>Form A</option><option>RCEP Certificate of Origin</option><option>China-Pakistan FTA Certificate</option><option>China-Chile FTA Certificate</option></select></label><label>原产国<input value={payload.originCountry} onChange={(event) => setPayloadField({ originCountry: event.target.value })} /></label><label className="field-wide">唛头<textarea rows={3} value={payload.shippingMarks} onChange={(event) => setPayloadField({ shippingMarks: event.target.value })} /></label></>}{inspectionCertificate && <><label>制造商<input value={payload.manufacturer} onChange={(event) => setPayloadField({ manufacturer: event.target.value })} /></label><label>制造商地址<input value={payload.manufacturerAddress} onChange={(event) => setPayloadField({ manufacturerAddress: event.target.value })} /></label><label>批次号<input value={payload.batchNumber} onChange={(event) => setPayloadField({ batchNumber: event.target.value })} /></label><label>检验标准<input value={payload.inspectionStandard} onChange={(event) => setPayloadField({ inspectionStandard: event.target.value })} /></label><label>检验日期<input type="date" value={payload.inspectionDate} onChange={(event) => setPayloadField({ inspectionDate: event.target.value })} /></label><label>检验地点<input value={payload.inspectionPlace} onChange={(event) => setPayloadField({ inspectionPlace: event.target.value })} /></label><label className="field-wide">检验结果<textarea rows={3} value={payload.inspectionResult} onChange={(event) => setPayloadField({ inspectionResult: event.target.value })} /></label></>}</div></>}
