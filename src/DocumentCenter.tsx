...
function CreateDocumentModal({
  ...
  const [type, setType] = useState<DocumentType>("commercial_invoice");
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}>
    <form className="modal-card document-create" onSubmit={submit} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="form-grid two-columns">
        <label>单证类型<select value={type} onChange={(event) => { const next = event.target.value as DocumentType; setType(next); setNumber(nextNumber(documents, next)); }}><option value="commercial_invoice">商业发票</option><option value="packing_list">详细装箱单</option><option value="trade_contract">外贸合同</option></select></label>
...
function Preview({ document, payload }: { document: TradeDocument; payload: DocumentPayload }) {
  ...
  return <div className={`document-paper ${packing ? "landscape" : ""}`}>
    <header><h2>{contract ? "SALES CONTRACT" : packing ? "DETAILED PACKING LIST" : "COMMERCIAL INVOICE"}</h2><p>{typeLabels[document.documentType]} · {document.number} · V{document.version}</p>{document.status === "draft" && <strong>DRAFT / 草稿</strong>}</header>
  ...
  return <div className="document-editor-shell">
    ...
    <div className="document-editor-layout">
      <div className="document-form-panel">
        ...
        {document.documentType === "commercial_invoice" && <label>银行资料<textarea rows={3} value={payload.bankDetails} onChange={(event) => setPayloadField({ bankDetails: event.target.value })} /></label>}
...
export function DocumentCenter(props: DocumentCenterProps) {
  ...
  return <section className="panel document-center">
    <div className="panel-heading"><div><h2>单证中心</h2><p>从业务单生成、校验并冻结可追溯的单证版本</p></div><button className="button button-primary" disabled={!props.cases.length} onClick={() => setCreating(true)}>新建单证</button></div>
    {!props.cases.length && <div className="empty-callout">请先建立业务单，再生成商业发票、详细装箱单或外贸合同。</div>}
    {message && <div className="document-message">{message}</div>}
    <div className="document-filters"><input placeholder="搜索单号、业务单或客户" value={query} onChange={(event) => setQuery(event.target.value)} /><select value={type} onChange={(event) => setType(event.target.value as typeof type)}><option value="all">全部类型</option><option value="commercial_invoice">商业发票</option><option value="packing_list">详细装箱单</option><option value="trade_contract">外贸合同</option></select><select value={status} onChange={(event) => setStatus(event.target.value as typeof status)}><option value="all">全部状态</option><option value="draft">草稿</option><option value="issued">已签发</option><option value="voided">已作废</option></select><span>{filtered.length} 个版本</span></div>
    <div className="document-history-list">{filtered.map((document) => <article className="document-history-card" key={document.id}><div className="document-type-mark">{prefixes[document.documentType]}</div><div className="document-history-main"><span className="eyebrow">{typeLabels[document.documentType]} · {document.businessCaseNumber}</span><h3>{document.number} <small>V{document.version}</small></h3><p>{document.customerName} · {document.issueDate} · 模板 {document.templateVersion}</p><div className="document-chips"><span className={`document-status ${document.status}`}>{statusLabels[document.status]}</span>{document.validationIssues.filter((item) => item.severity === "error").length > 0 && <span className="validation-chip">{document.validationIssues.filter((item) => item.severity === "error").length} 个错误</span>}{document.pdfSha256 && <span>PDF {document.pdfSha256.slice(0, 10)}…</span>}</div></div><div className="document-card-actions"><button onClick={() => setEditing(document)}>{document.status === "draft" ? "编辑" : "查看"}</button>{document.pdfPath && <button onClick={() => void props.onOpenPdf(document.id)}>打开 PDF</button>}{document.status === "issued" && <button onClick={() => void newVersion(document)}>创建新版本</button>}{document.status === "issued" && <button className="danger-link" onClick={() => void voidDocument(document)}>作废</button>}</div></article>)}</div>
