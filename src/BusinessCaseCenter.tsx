...
export function BusinessCaseCenter({ cases, customers, products, onSave, onArchive }: BusinessCaseCenterProps) {
  ...
  return <>
    <section className="panel business-case-panel">
      ...
      <div className="table-wrap"><table><thead><tr><th>业务单号</th><th>客户</th><th>状态</th><th>贸易术语</th><th>计划发货</th><th>金额</th><th>操作</th></tr></thead><tbody>{filteredCases.map((item) => <tr key={item.id}><td><strong>{item.number}</strong><small className="table-subtitle">{item.lines.length} 个产品行</small></td><td>{item.customerName}</td><td><span className="case-stage">{stageLabels[item.stage]}</span></td><td>{item.incoterm || "—"}</td><td>{item.shipmentDate || "—"}</td><td>{formatMoney(item.totalAmountMinor, item.currency)}</td><td className="row-actions"><button onClick={() => setEditing(item)}>编辑</button><button onClick={() => archive(item)}>归档</button></td></tr>)}</tbody></table></div>
